...
def augmentation(img, bbox, data_split, enforce_flip=None):
    ...
    #img, trans, inv_trans = generate_patch_image(img, bbox, 1., 0., False, cfg.input_img_shape)
...
def process_db_coord(joint_img, joint_cam, joint_valid, do_flip, img_shape, flip_pairs, img2bb_trans, rot, src_joints_name, target_joints_name):
    ...
    # check truncation
    joint_trunc = joint_valid * ((joint_img[:,0] >= 0) * (joint_img[:,0] < cfg.input_img_shape[1]) * \
                (joint_img[:,1] >= 0) * (joint_img[:,1] < cfg.input_img_shape[0]) * \
