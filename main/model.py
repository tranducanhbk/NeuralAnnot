...
from nets.loss import CoordLoss, ParamLoss, RegularizeLoss
...
class Model(nn.Module):
    def __init__(self, networks):
        ...
        self.regular_loss = RegularizeLoss()
    ...
    def forward_backbone(self, inputs, backbone):
        img_feat = backbone(inputs['img'])
        return img_feat
    
    def forward_rotation_net(self, img_feat, rotation_net):
        # parameter estimation
        if cfg.parts == 'body':
            root_pose, shape_param, cam_param, vposer_para = rotation_net(img_feat)
            ...
            return root_pose, shape_param, cam_trans, vposer_para
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        if cfg.parts == 'body':
            ...
            smpl_body_pose = torch.cat((smpl_body_pose, torch.zeros((batch_size,2*3)).cuda().float()),1) 
